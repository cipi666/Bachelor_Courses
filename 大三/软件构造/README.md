...
### <center>UML类图的画法
在UML类图中，类使用包含类名、属性(field) 和方法(method) 且带有分割线的矩形来表示,各个类之间的关系可以通过下图的连线表示：
...
- 案例：如果使用 Iterator 来遍历集合中元素，一旦不再使用 List 转而使用 Set 来组织数据，那遍历元素的代码不用做任何修改；如果使用 for 来遍历，那所有遍历此集合的算法都得做相应调整,因为List有序,Set无序,结构不同,他们的访问算法也不一样
- 在循环的时候，采用Iterator可以删除元素；而无法在for循环中删除元素
